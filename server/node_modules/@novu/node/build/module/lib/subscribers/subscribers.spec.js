...
import { ChannelTypeEnum, MessageActionStatusEnum, ButtonTypeEnum, MarkMessagesAsEnum, } from '@novu/shared';
const mockConfig = {
    apiKey: '1234',
};
jest.mock('axios');
describe('test use of novus node package - Subscribers class', () => {
    ...
    test('should mark message action as seen', async () => {
        mockedAxios.post.mockResolvedValue({});
        await novu.subscribers.markMessageActionSeen('test-action-type-sub', 'message-123', ButtonTypeEnum.PRIMARY, { status: MessageActionStatusEnum.DONE });
        expect(mockedAxios.post).toHaveBeenCalled();
    ...
    test('should mark all subscriber messages as read', async () => {
        mockedAxios.post.mockResolvedValue({});
        await novu.subscribers.markAllMessagesAs('test-action-type-sub', MarkMessagesAsEnum.READ);
        expect(mockedAxios.post).toHaveBeenCalled();
    ...
    test('should mark all subscriber messages as read for feed', async () => {
        mockedAxios.post.mockResolvedValue({});
        await novu.subscribers.markAllMessagesAs('test-action-type-sub', MarkMessagesAsEnum.READ, 'feed-123');
        expect(mockedAxios.post).toHaveBeenCalled();
        expect(mockedAxios.post).toHaveBeenCalledWith('/subscribers/test-action-type-sub/messages/mark-all', {
            markAs: MarkMessagesAsEnum.READ,
